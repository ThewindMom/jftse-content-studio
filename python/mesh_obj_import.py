...
from mesh_codec import DecodedMesh, decode_member, write_positions_into_dat
...
def _mesh_layout(mesh: DecodedMesh) -> tuple[int, int, int]:
    """Return (count, offset, stride) from DecodedMesh camelCase fields."""
    return mesh.vertexCount, mesh.vertexOffset, mesh.vertexStride
...
def import_obj_into_dat(
    ...
    count, offset, stride = _mesh_layout(mesh)
    positions = parse_obj_positions(
        obj_path.read_text(encoding="utf-8", errors="replace")
    )
    if not positions:
    ...
    patched = write_positions_into_dat(data, offset, positions, stride=stride)
